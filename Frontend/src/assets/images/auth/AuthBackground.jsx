...
// project import
import { ThemeDirection, ThemeMode } from 'config';
import { getCouncilList } from 'services/masterServices/council-details/council-details-service';
  ...
  useEffect(() => {
    const loadCouncilInfo = async () => {
      try {
        ...
        console.log(":inbox_tray: Fetched council data:", data);
        console.log(":camera_with_flash: NPImage URL:", data?.NPImage);
        if (data?.NPImage) {
          const fullUrl = data.NPImage.startsWith('http')
            ? data.NPImage
            : `http://localhost:4000/${data.NPImage}`;
          console.log(":white_check_mark: Full image URL for background:", fullUrl);
          setBgImage(fullUrl);
        } else {
          console.warn(":warning: No NPImage found in backend data");
        }
      } catch (err) {
        console.error(":x: Error loading council info:", err);
      }
    };
    loadCouncilInfo();
  }, []);
